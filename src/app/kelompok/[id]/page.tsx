...
export default function InputJurnalPage({ params }: { params: Promise<{ id: string, siswaId: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {/* BODY CONTENT 
          Padding Bottom Besar: pb-40
      */}
      <main className="flex-1 max-w-2xl mx-auto p-4 space-y-4 pb-40 w-full">
                        ...
                        {isOpen && (
                            <div className="p-4 bg-slate-50 border-t border-slate-100 animate-in slide-in-from-top-2">
                                <div className="space-y-4">
                                    ...
                                    {/* --- PERUBAHAN DI SINI: TIDAK LAGI FIXED BOTTOM --- */}
                                    <div className="fixed bottom-24 left-4 right-4 z-50">
                                         <button 
                                            onClick={() => handleSaveItem(target.id)}
                                            disabled={savingId === target.id}
                                            className="w-full py-3 bg-blue-600 text-white font-bold rounded-xl shadow-lg shadow-blue-200 active:scale-95 transition-all flex justify-center items-center gap-2 border-2 border-white/20"
                                        >
                                    ...
                                    {/* DUMMY SPACER AGAR TIDAK TERTUTUP TOMBOL YANG MELAYANG */}
